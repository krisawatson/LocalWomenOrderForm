...
.directive("signaturePad", ['$interval', '$timeout', '$window', SignatureDirective]);

function SignatureDirective($interval, $timeout, $window) {
	...
	return {
		restrict: 'EA',
		replace: true,
		template: '<div class="signature{{type}}" style="width: 100%; max-width:{{width}}px; height: 100%; max-height:{{height}}px;"><canvas style="display: block; margin: 0 auto;" data-ng-mouseup="onMouseup()" data-ng-mousedown="notifyDrawing({ drawing: true })"></canvas><div>{{type}} Signature</div></div>',
		scope: {
			accept: '=?',
			clear: '=?',
			dataurl: '=?',
